...
class PerformancePrefs
{
     ...
     * Get boolean value
     */
    public static function getBoolean($key, $defvalue = false) : bool
    {
     ...
     * Get Integer value
     */
    public static function getInteger($key, $defvalue = false) : int
    {
...
class PearDatabase
{
    ...
    {
        return (stripos($this->dbType, 'mysqli') === 0);
    }
    ...
    public function checkError($msg = '', $dieOnError = false)
    {
        ...
        if ($this->dieOnError || $dieOnError) {
            ...
            $log->error("ADODB error " . $msg . "->[" . $this->database->ErrorNo() . "]" . $this->database->ErrorMsg());
            die();
        } else {
            $log->error("ADODB error " . $msg . "->[" . $this->database->ErrorNo() . "]" . $this->database->ErrorMsg());
        }
